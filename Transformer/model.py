...
t = MyDataSet(enc_inputs, dec_inputs, dec_outputs)
...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    # [batch_size, 1, len_k], 如果是真就表示被mask
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        """
        Q: [batch_size, n_heads, len_q, d_k] len_q表示单词数量
        K: [batch_size, n_heads, len_k, d_k] len_k表示单词数量
        ...
        scores.masked_fill_(attn_mask, -1e9)#-1e9 = −1,000,000,000

        attn = nn.Softmax(dim=-1)(scores)  # 对最后一个维度(v)做softmax,Softmax 是对矩阵的每一行进行 Softmax，即每一行的和都变为 1.但维度不变化
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.src_emb = nn.Embedding(src_vocab_size, d_model)  # token Embedding,0为中文词袋大小（不是中文句子长度）
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        for layer in self.layers:
            ...
            # Decoder的Block是上一个Block的输出dec_outputs（变化）和Encoder网络的输出enc_outputs（固定）,第一个dec_outputs就是dec_inputs
